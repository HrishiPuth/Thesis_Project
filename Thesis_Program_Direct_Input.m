...
block_length = [1.06 0.495 0.556];
block_mass = [1024 242 350];
block_cg = [0.559 1.272 1.670];
...
n_mounts = 4;
  ...
  bracket_stiffness = [102739720 102739720 102739720 102739720];
  mount_distance = [0.001 1.06 1.45 2.055];
  mount_stiffness = [960000 2*960000 2*960000 960000];
  mount_bottoming_out = [0.05 0.005 0.005 0.005];
...
%% function for calculating k11, k12, k21 and k22
...
%calculating displacements in springs other than the first and last springs
 for i = 1:n_mounts-2
   ...
   pilot_disp(i) = pilot_displacement_matrix(1)+(pilot_displacement_matrix(2)-pilot_displacement_matrix(1))*(mount_distance(i+1)-mount_distance(1))/L;
...
if any(difference <= 0) == 0
  ...
  disp(reaction_force)
  disp('NO BOTTOMING-OUT OCCURING')
endif
 ...
 % Solve the equations for the two cases where displacements at the ends are known
  ...
  % Solve the equations for x1, x2, and g in each case
      ...
      for i = 1:length(z)
          ...
          A3 = [stiffness_matrix(1,1),stiffness_matrix(1,2), (-mass_matrix(1));stiffness_matrix(2,1), stiffness_matrix(2,2), (-mass_matrix(2)); (1-(mount_distance(i+1)-mount_distance(1))/L), ((mount_distance(i+1)-mount_distance(1))/L),0];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CALCULATING DISPLACEMENTS AT FIRST BOTTOM OUT  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = 1:(n_mounts-2)
      disp_1stBO(i) = displacements_1st_BO(1)+(displacements_1st_BO(2)-displacements_1st_BO(1))*(mount_distance(i+1)-mount_distance(1))/L; %calculates the displacements of the mounts in between the first and last ones
    endfor
    %making full displacement vector
    ...
    disp(['This mount is mount ',num2str(first_bottoming_out_mount)])
    flag = 1;
  endif
...
%%%%%%%%%%%%%%%%%%%%%%%%%  RECALCULATING THE DISPLACEMENTS BEYOND THE CURRENT BOTTOM OUT TO CHECK FOR FURTHER BOTTOM OUTS %%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = 1:n_mounts-2
      rem_check_disp(i) = extra_displacement(1)+(extra_displacement(2)-extra_displacement(1))*(mount_distance(i+1)-mount_distance(1))/L; %finds the values of displacements between the mounts at the ends
    endfor
